...
public class CeProcessor170_1 {
	
	private static final Logger log = Logger.getLogger(CeProcessor170_1.class);
	private ExecPool pool = new ExecPool();
	public CeProcessor170_1() {
		super();
	}
	 ...
	 *@throws Exception
	 */
	public int getInnerVersion(String batchId) throws Exception {
		int innerVer = 0;
		String sql = null;
	 ...
	 *@return
	 */
	public Results queryBatchs(String basePath, List<AttrInfo> attrList, String appCode) {
		long startTime = System.currentTimeMillis();
	 ...
	 *@return
	 */
	public int existsBatch(List<AttrInfo> attrList, String appCode) {
		StringBuilder searchSql = new StringBuilder();
		searchSql.append(CeSql.QUERY_BATCHS).append(" WHERE f.APP_CODE='").append(appCode).append("'");
		if (attrList != null && attrList.size() > 0) {
			for (int i = 0; i < attrList.size(); i++) {
