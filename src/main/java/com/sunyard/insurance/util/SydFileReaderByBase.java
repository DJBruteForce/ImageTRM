...
public class SydFileReaderByBase implements SydFileReader{
	public SydFileReaderByBase() {
		super();
	}
	/**
	 * 
	...
	public DocBean readDocBean(File sydFile) throws DocumentException {
		...
		List<?> propList = docInfo.element("DOC_EXT").elements();
		...
		if (docInfo != null) {
			...
			String busiNo = XmlUtil.getTrimText(docInfo.element("BUSI_NUM"));
			...
			String modUser = XmlUtil.getTrimText(docInfo.element("MODIFY_USER"));
			String modDate = XmlUtil.getTrimText(docInfo.element("MODIFY_DATE"));
		...
		if (propList != null) {
			...
			for (int i = 0; i < propList.size(); i++) {
				...
				String code = extElement.attributeValue("ID");
	...
	public Result getQueryResult(File batchFile, String basePath) throws Exception {
		...
		String busiNo = XmlUtil.getNodeText(document, "/doc/DocInfo/BUSI_NUM");
